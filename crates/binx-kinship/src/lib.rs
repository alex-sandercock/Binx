...
pub enum KinshipMethod {
    /// VanRaden (2008) additive relationship matrix
    /// K = M'M / (m * sum(p(1-p))) where M is centered by ploidy * p
    #[default]
    VanRaden,
    /// GWASpoly-style kinship (centered, normalized to unit diagonal mean)
    /// K = MM' / mean(diag(K)) where M is centered by column means
    GWASpoly,
}
...
impl KinshipMethod {
    pub fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            "vanraden" => Ok(KinshipMethod::VanRaden),
            "gwaspoly" => Ok(KinshipMethod::GWASpoly),
            _ => Err(anyhow!("Unknown kinship method '{}'. Use 'vanraden' or 'gwaspoly'", s)),
        }
    }
}

/// Compute kinship matrix from a biallelic genotype TSV and write to TSV.
pub fn run_kinship(geno_path: &str, ploidy: u8, method: KinshipMethod, out_path: &str) -> Result<()> {
    let geno = load_genotypes_biallelic_from_tsv(geno_path, ploidy)?;
    let kin = compute_kinship(&geno, method)?;
    write_kinship_tsv(out_path, &kin)
}

/// Compute kinship matrix using the specified method
pub fn compute_kinship(geno: &GenotypeMatrixBiallelic, method: KinshipMethod) -> Result<KinshipMatrix> {
    match method {
        KinshipMethod::VanRaden => compute_kinship_vanraden(geno),
        KinshipMethod::GWASpoly => compute_kinship_gwaspoly(geno),
    }
}
...
/// Compute GWASpoly-style kinship matrix.
/// This matches R/GWASpoly's set.K() function:
/// 1. Center the genotype matrix (subtract column means)
/// 2. K = M @ M' (individuals × individuals)
/// 3. K = K / mean(diag(K)) - normalize to have unit diagonal on average
pub fn compute_kinship_gwaspoly(geno: &GenotypeMatrixBiallelic) -> Result<KinshipMatrix> {
    let n_markers = geno.marker_ids.len();
    let n_samples = geno.sample_ids.len();
    ...
    // Compute column means (per marker)
    ...
    for i in 0..n_markers {
        let mean = col_means[i];
        for j in 0..n_samples {
            let val = geno.dosages[(i, j)];
            centered[(j, i)] = if val.is_finite() {
                val - mean
            } else {
