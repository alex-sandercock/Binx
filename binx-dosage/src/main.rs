...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 3 {
        eprintln!("Usage: binx-dosage <csv_file> <ploidy> [--mode <auto|updog|fast>] [--verbose]");
        ...
        eprintln!("  --mode updog    Full validation (5 starts like R package)");
    ...
    let mode = if let Some(mode_idx) = args.iter().position(|arg| arg == "--mode") {
        ...
        match mode_str.as_str() {
            ...
            "fast" => FitMode::Fast,
            _ => {
                eprintln!("Invalid mode: {}. Use 'auto', 'updog', or 'fast'", mode_str);
                std::process::exit(1);
            }
        }
    } else {
